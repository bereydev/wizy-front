...
interface Props {
	...
	locale?: string
	// monthNameFormat:string as PropType<DateTimeFormatOption>
	// weekdayNameFormat:string as PropType<DateTimeFormatOption>
...
const currentDragItem = ref<INormalizedCalendarItem | null>(null)
const dateSelectionOrigin = ref<Date | null>(null)
const currentHoveredItemId = ref<string>("")
...
*/
const displayLocale = computed<string>(() =>
	locale.value || CalendarMath.getDefaultBrowserLocale()
)
...
*/
const defaultedShowDate = computed<Date>(() =>
	showDate ? CalendarMath.dateOnly(showDate.value) : CalendarMath.today()
)
...
const periodStart = computed<Date>(() =>
	CalendarMath.beginningOfPeriod(defaultedShowDate.value, displayPeriodUom.value, startingDayOfWeek.value)
)

const periodEnd = computed<Date>(() =>
	CalendarMath.addDays(CalendarMath.incrementPeriod(periodStart.value, displayPeriodUom.value, displayPeriodCount.value), -1)
)

const periodStartCalendarWeek = computed<number>(() => {
	const firstWeekStarts = CalendarMath.beginningOfWeek(CalendarMath.beginningOfPeriod(periodStart.value, "year", 0), startingDayOfWeek.value)
	const periodWeekStarts = CalendarMath.beginningOfWeek(periodStart.value, startingDayOfWeek.value)
...
*/
const displayFirstDate = computed<Date>(() =>
	CalendarMath.beginningOfWeek(periodStart.value, startingDayOfWeek.value)
)

const displayLastDate = computed<Date>(() =>
	CalendarMath.endOfWeek(periodEnd.value, startingDayOfWeek.value)
)
...
const monthNames = computed<Array<string>>(() =>
	CalendarMath.getFormattedMonthNames(displayLocale.value, monthNameFormat.value)
)

const weekdayNames = computed<Array<string>>(() =>
	CalendarMath.getFormattedWeekdayNames(displayLocale.value, weekdayNameFormat.value, startingDayOfWeek.value)
)

// Ensure all item properties have suitable default
const fixedItems = computed<Array<INormalizedCalendarItem>>(() =>
	!items.value ? [] : items.value.map((item) => CalendarMath.normalizeItem(item, item.id === currentHoveredItemId.value))
)
// Period that today's date sits within
const currentPeriodStart = computed(() =>
	CalendarMath.beginningOfPeriod(CalendarMath.today(), displayPeriodUom.value, startingDayOfWeek.value)
)

const currentPeriodEnd = computed(() =>
	CalendarMath.addDays(CalendarMath.incrementPeriod(currentPeriodStart.value, displayPeriodUom.value, displayPeriodCount.value), -1)
)

// Creates the HTML to render the date range for the calendar header.
const periodLabel = computed<string>(() =>
	CalendarMath.formattedPeriod(periodStart.value, periodEnd.value, displayPeriodUom.value, monthNames.value)
)

const currentPeriodLabelFinal = computed(() => {
	const c = currentPeriodStart.value
	const s = periodStart.value
	if (!currentPeriodLabel.value) return CalendarMath.formattedPeriod(c, currentPeriodEnd.value, displayPeriodUom.value, monthNames.value)
	if (currentPeriodLabel.value === "icons") return currentPeriodLabelIcons[Math.sign(c.getTime() - s.getTime()) + 1]
	return currentPeriodLabel
})

const headerProps = computed<IHeaderProps>(() => {
	return {
		...
		previousFullPeriod: getIncrementedPeriod(-displayPeriodCount.value),
		nextFullPeriod: getIncrementedPeriod(displayPeriodCount.value),
...
watchEffect(
	() => {
		if (periodChangedCallback.value) {
			emit("period-changed")
			periodChangedCallback.value(periodRange.value, "watch")
		}
	}
)
...
function onClickDay(day: Date, windowEvent: Event): void {
	if (disablePast && CalendarMath.isInPast(day)) return
	if (disableFuture && CalendarMath.isInFuture(day)) return
	emit("click-date", day, findAndSortItemsInDateRange(day, day), windowEvent)
}

function onClickItem(calendarItem: ICalendarItem, windowEvent: Event): void {
...
*/
function getColumnDOWClass(dayIndex: number): string {
	return "dow" + ((dayIndex + startingDayOfWeek.value) % 7)
}
...
*/
function getIncrementedPeriod(count: number): Date | null {
	const newStartDate = CalendarMath.incrementPeriod(periodStart.value, displayPeriodUom.value, count)
	const newEndDate = CalendarMath.incrementPeriod(newStartDate, displayPeriodUom.value, displayPeriodCount.value)
	if (disablePast && newEndDate <= CalendarMath.today()) return null
	if (disableFuture && newStartDate > CalendarMath.today()) return null
	return newStartDate
}
...
function onMouseEnterItem(calendarItem: ICalendarItem, windowEvent: Event): void {
	currentHoveredItemId.value = calendarItem.id
	if (doEmitItemMouseEvents) {
		emit("item-mouseenter", calendarItem, windowEvent)
	}
}

function onMouseLeaveItem(calendarItem: ICalendarItem, windowEvent: Event): void {
...
function onDragDateStart(day: Date, windowEvent: DragEvent): boolean {
	if (!enableDateSelection) return false
...
function onDragItemStart(calendarItem: INormalizedCalendarItem, windowEvent: DragEvent): boolean {
	...
	currentDragItem.value = calendarItem
	// Reset date selection origin so the onenter events aren't confused
	dateSelectionOrigin.value = null
	// Allow the calling application to add additional functionality.
	emit("drag-start", calendarItem, windowEvent)
	return true
}

function handleDragEvent(bubbleEventName: "drag-over-date" | "drag-enter-date" | "drag-leave-date" | "drop-on-date", bubbleParam: any, windowEvent: Event): boolean {
	if (!enableDragDrop) return false
	...
	emit(bubbleEventName, currentDragItem, bubbleParam, windowEvent)
	return true
}

function onDragOver(day: Date, windowEvent: Event): void {
	handleDragEvent("drag-over-date", day, windowEvent)
}

function onDragEnter(day: Date, windowEvent: Event) {
	if (enableDateSelection && dateSelectionOrigin.value) {
...
function onDragLeave(day: Date, windowEvent: Event): void {
	// User is selecting dates, not items. No emit.
	if (enableDateSelection && selectionStart) return
	if (!handleDragEvent("drag-leave-date", day, windowEvent)) return
	const el = windowEvent.target as HTMLElement
	el.classList.remove("draghover")
}

function onDrop(day: Date, windowEvent: Event): void {
	if (enableDateSelection && dateSelectionOrigin.value) {
		// User is selecting dates, not items.
...
function emitDateSelection(eventName: "date-selection" | "date-selection-start" | "date-selection-finish", toDate: Date, windowEvent: Event): void {
	if (!dateSelectionOrigin.value) return
	emit(eventName, toDate <= dateSelectionOrigin.value ? [toDate, dateSelectionOrigin, windowEvent] : [dateSelectionOrigin, toDate, windowEvent])
}
...
function itemComparer(a: INormalizedCalendarItem, b: INormalizedCalendarItem) {
...
function findAndSortItemsInWeek(weekStart: Date): Array<INormalizedCalendarItem> {
	// Return a list of items that INCLUDE any portion of a given week.
	return findAndSortItemsInDateRange(weekStart, CalendarMath.addDays(weekStart, 6))
}

function findAndSortItemsInDateRange(startDate: Date, endDate: Date): Array<INormalizedCalendarItem> {
	...
	return fixedItems.value.filter((item) => item.endDate >= startDate && CalendarMath.dateOnly(item.startDate) <= endDate).sort(itemComparer)
}

function dayHasItems(day: Date): boolean {
	return !!fixedItems.value.find((d) => d.endDate >= day && CalendarMath.dateOnly(d.startDate) <= day)
}

function dayIsSelected(day: Date): boolean {
	if (!selectionStart.value || !selectionEnd.value) return false
	if (day < CalendarMath.dateOnly(selectionStart.value)) return false
	if (day > CalendarMath.dateOnly(selectionEnd.value)) return false
	return true
}

function getWeekItems(weekStart: Date): Array<INormalizedCalendarItem> {
	...
	const results: Array<INormalizedCalendarItem> = []
...
*/
function getFormattedTimeRange(item: INormalizedCalendarItem): string {
	const startTime = '<span class="startTime">' + CalendarMath.formattedTime(item.startDate, displayLocale.value, timeFormatOptions.value) + "</span>"
	let endTime = ""
	if (!CalendarMath.isSameDateTime(item.startDate, item.endDate)) {
		endTime =
			'<span class="endTime">' +
			CalendarMath.formattedTime(item.endDate, displayLocale.value, timeFormatOptions.value) + "</span>"
	}
	return startTime + endTime
}

function getItemTitle(item: INormalizedCalendarItem): string {
	if (!showTimes) return item.title
	return getFormattedTimeRange(item) + " " + item.title
}

function getItemTop(item: INormalizedCalendarItem): string {
	// Compute the top position of the item based on its assigned row within the given week.
	const r = item.itemRow
	const h = itemContentHeight
	const b = itemBorderHeight
